...
/*
 * kernel/kernel.h : kernel.cmm の外部インタフェース
 *
 ...
 *   名前   : zmbProc
 *   戻り値 : なし(この関数は戻らない)
 *   解説   : zmbProc ルーチンは、呼び出したプロセスをゾンビにする。
 *            その後、他のプロセスにディスパッチする。
 *            PM の EXIT 処理終了後、EXITするプロセス自身が呼出す。
 */
public void zmbProc();
 ...
 *   名前   : waitChild
 *   戻り値 : 子プロセスの PCB
 *   解説   : waitChild は現在のプロセスの子プロセスを探す
 *            ZOMBIE になったものを優先的に返す
 *            子プロセスが全く見つからない場合は null を返す
